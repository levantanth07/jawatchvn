                ...
                <div class="x_panel">
                    <div class="x_content">
                        ...
                        <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left"
                              method="post" action="{{ route('backend.post.update', $item->id) }}" enctype="multipart/form-data">
                            @csrf
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <select class="form-control js-states" name="post_type" id="post_type">
                                        @foreach($postTypes as $key => $value)
                                            <option @if($key == $item->post_type) {{ 'selected' }} @endif value="{{ $key }}">{{ $value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="item form-group">
                                ...
                                <div class="col-md-6 col-sm-6">
                                    <select class="form-control" name="status" id="status">
                                        <option @if($item->post_type == 1) {{ 'selected' }} @endif value="1">Hiển thị</option>
                                        <option @if($item->post_type == 2) {{ 'selected' }} @endif value="2">Ẩn</option>
                                    </select>
                                </div>
                            </div>
